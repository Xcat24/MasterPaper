...
model.add(pool2)
conv3 = Convolution2D(nb_filters_3, kernel_size_3[0], kernel_size_3[1], border_mode='same')
model.add(conv3)
model.add(Activation('relu'))

pool3 = MaxPooling2D(pool_size=pool_size_3)
model.add(pool3)

conv4 = Convolution2D(nb_filters_4, kernel_size_4[0], kernel_size_4[1], border_mode='same')
model.add(conv4)
model.add(Activation('relu'))

pool4 = MaxPooling2D(pool_size=pool_size_4)
model.add(pool4)

model.add(Permute((2,1,3)))
model.add(Reshape((12,64*118)))
...
modelname = '4cnn2lstm_true_max_relu'
